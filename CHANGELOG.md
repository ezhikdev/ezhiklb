...
- Для каждой записи маршрутизации добавлен необязательный лимит скорости в
  Мбит/с. Он применяется локально на каждой ноде, объединяет TCP и UDP одной
  записи в общий бюджет и ограничивает исходное и ответное направления
  независимо одним и тем же значением.
- Ограничение строго opt-in: старые профили загружаются с выключенным лимитом,
  а ноды без активных или ранее сохранённых лимитов не получают ни одной
  изменяющей команды `tc`/mangle. Публикация ограниченного профиля на агент
  старее `1.1.0` блокируется панелью и API.
- Лимитер использует отдельные `clsact` egress-фильтры и собственную цепочку
  `EZHIKLB-RATE`; корневую qdisc и чужие правила ноды он не заменяет.
- Перетаскивание записей переработано: карточка свободно следует за указателем
  и после отпускания плавно занимает место в списке. Такой же сохраняемый
  порядок добавлен профилям и нодам, включая управление стрелками с клавиатуры
  и учёт `prefers-reduced-motion`.
- Диагностика агента сообщает доступность `tc`, число активных ограниченных
  записей и счётчик отброшенных лимитером пакетов.

## 1.0.10

- Исправлено расхождение версии в `/etc/ezhiklb/version`: однокнопочное
  обновление из панели меняло только сам бинарник агента и никогда не
  обновляло этот файл — он оставался равен версии самой первой установки,
  даже если нода потом обновлялась через панель много раз подряд. Из-за
  этого при следующем ручном запуске `install.sh` (например, апгрейд в обход
  панели) показывалась неверная «Existing EzhikLB X detected». На реальное
  поведение это не влияло — чисто информационная нестыковка.
- Self-update теперь тоже пишет актуальную версию в `/etc/ezhiklb/version`
  после успешной замены бинарника (best-effort — если запись не удалась,
  само обновление всё равно считается успешным).
- Для этого systemd-юниту агента разрешена запись в `/etc/ezhiklb`
  (`ReadWritePaths`, добавлено к уже существующим путям для `state.json` и
  бинарника). Нодам, которые обновлялись только через панель (без разового
  прогона `install.sh`), нужно один раз получить этот фикс через полный
  `install.sh`, чтобы права на systemd-юните обновились — после этого все
  дальнейшие self-update из панели будут держать файл версии в актуальном
  состоянии сами.

## 1.0.9

- Исправлена серьёзная регрессия из `1.0.8`: продление таймаута UDP до 24
  часов раздувало собственную таблицу соединений IPVS (`/proc/net/ip_vs_conn`)
  накопленным трафиком за целые сутки вместо нескольких минут. Агент читает
  этот файл на каждый heartbeat для метрики активных IP — с раздутой таблицей
  это давало устойчивую нагрузку 40-50%+ CPU на магистральных нодах, а
  conntrack приближался к своему лимиту записей.
- Таймауты теперь разделены осознанно: у самого IPVS-соединения таймаут
  вернулся к разумным 300 секундам (`ipvsadm --set`) — оно не обязано жить
  долго, потому что при простое клиента маршрут на тот же backend
  восстанавливает Affinity/persistence-шаблон независимо от этой записи. А
  таймаут conntrack (`nf_conntrack_udp_timeout_stream`) остаётся 24 часа —
  именно от него зависит правило `EZHIKLB-FORWARD` (`--ctstate
  ESTABLISHED,RELATED`) на обратный трафик, и именно это в итоге чинило исходный
  разрыв сессии.
- `nf_conntrack_max` поднят до 2 000 000, чтобы conntrack не начал сбрасывать
  новые соединения, пока записи по-прежнему живут до суток.
- Агент теперь сканирует `/proc/net/ip_vs_conn` не на каждый heartbeat (15
  сек), а не чаще раза в 30 секунд — дополнительный запас для нагруженных нод.

## 1.0.8

- Исправлен незакрытый баг из «Confirmed alpha.5 findings»: разрыв UDP-сессии
  (например, VPN-клиента) после блокировки телефона в среднем дольше 5 минут.
  Причина — дефолтные таймауты ядра Linux короче, чем реальный простой:
  таймаут UDP-соединения в IPVS по умолчанию ровно 300 секунд, а
  `nf_conntrack_udp_timeout_stream` обычно 120-180 секунд — оба заметно
  меньше того, что уже обещает Affinity (до 24 часов).
- Агент теперь явно расширяет оба таймаута до 24 часов — 86400 секунд, это
  верхняя граница из пресетов Affinity в панели (`ipvsadm --set` и сисктлы
  `nf_conntrack_udp_timeout*`) — при каждом применении конфигурации и при
  восстановлении после перезагрузки, чтобы состояние NAT/IPVS переживало
  реалистичный простой клиента вместо разрыва сессии и повторного хендшейка.
  Это глобальный параметр ядра на всю ноду, не привязанный к конкретному
  значению Affinity отдельного listener'а — Affinity по-прежнему отвечает
  только за маршрутизацию на тот же backend.
