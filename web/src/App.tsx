import { Activity, ArrowDown, ArrowUp, Boxes, CheckCircle2, ChevronDown, ChevronRight, CircleGauge, Clock3, Copy, Cpu, HeartPulse, Hexagon, History, LoaderCircle, LogOut, MemoryStick, Network, Pencil, Plus, Power, RefreshCw, Save, Server, Settings, ShieldCheck, Trash2, Users } from "lucide-react"
...
function NodeRow({ node, profile }: { node: NodeInfo; profile?: Profile }) { const synced = node.desired_revision === node.applied_revision; return <div className="node-row"><div className="node-name"><div className={`node-avatar ${node.status === "online" ? "node-avatar--online" : ""}`}><Server /></div><div><strong>{node.name}</strong><span>{node.last_error || profile?.name || "Без профиля"}</span><NodeMetricsStrip node={node} compact /></div></div><div className="node-row__revision"><span className="mono">{node.applied_revision}/{node.desired_revision}</span><Badge tone={node.status === "error" ? "danger" : synced && node.status === "online" ? "success" : node.status === "connecting" || node.status === "deleting" ? "warning" : "neutral"}>{node.status === "error" ? "ошибка" : node.status === "connecting" ? "подключается" : synced && node.status === "online" ? "применено" : nodeStatusLabel(node.status)}</Badge></div></div> }
