...
export default function App() {
  ...
  const load = useCallback(async () => {
    try {
      const [nextStatus, nextProfiles, nextNodes, nextHealth] = await Promise.all([api.status(), api.profiles(), api.nodes(), api.health()])
      setStatus(nextStatus); setProfiles(nextProfiles); setNodes(nextNodes); setHealth(nextHealth); setAuthenticated(true); setError("")
    } catch (reason) {
  ...
  return <><a className="skip-link" href="#main-content">Перейти к содержимому</a><div className="app-shell">
    ...
    <main id="main-content" tabIndex={-1} className="main-content">
      ...
      {page === "overview" && <Overview status={status} nodes={nodes} profiles={profiles} navigate={setPage} />}
    ...
    {(editing || creating) && <ProfileDialog existing={editing} onClose={() => { setEditing(null); setCreating(false) }} onSaved={async () => { setEditing(null); setCreating(false); await load() }} />}
  </div></>
}
...
function Overview({ status, nodes, profiles, navigate }: { status: Status | null; nodes: NodeInfo[]; profiles: Profile[]; navigate: (page: Page) => void }) {
  return <div className="page"><PageHeader eyebrow="Control plane" title="Обзор" description="Состояние инфраструктуры EzhikLB в одном месте." />
    <div className="metrics-grid"><Metric label="Ноды онлайн" value={`${status?.online_nodes ?? 0}/${status?.nodes ?? 0}`} icon={<Server />} tone="success" /><Metric label="Профили" value={String(status?.profiles ?? 0)} icon={<Boxes />} /><Metric label="Активные записи" value={String(status?.listeners ?? 0)} icon={<Network />} /><Metric label="Data plane" value="IPVS" icon={<Activity />} /></div>
...
function ProfileDialog({ existing, onClose, onSaved }: { existing: { profile: Profile; revision: Revision } | null; onClose: () => void; onSaved: () => Promise<void> }) {
  const [name, setName] = useState(existing?.profile.name ?? "Новый профиль")
  const [description, setDescription] = useState(existing?.profile.description ?? "")
  const [config, setConfig] = useState<ProfileConfig>(existing?.revision.config ?? emptyConfig())
  ...
  return <Dialog wide title={existing ? `Редактирование · ${existing.profile.name}` : "Новый профиль"} description={existing ? `Сохранение создаст revision ${existing.profile.current_revision + 1}` : "Настройте health-check и маршруты."} onClose={onClose}><div className="dialog__body"><div className="profile-basics"><Field label="Название"><Input value={name} onChange={(e) => setName(e.target.value)} /></Field><Field label="Описание"><Input value={description} onChange={(e) => setDescription(e.target.value)} /></Field></div><ProfileEditor initial={config} onChange={setConfig} />{error && <div className="validation-error" role="alert">{error}</div>}</div><div className="dialog__footer"><Button variant="ghost" onClick={onClose}>Отмена</Button><Button disabled={busy || !name.trim()} onClick={save}><Save data-icon="inline-start" />{busy ? "Публикую…" : "Опубликовать ревизию"}</Button></div></Dialog>
}
...
function NodeRow({ node, profile }: { node: NodeInfo; profile?: Profile }) { const synced = node.desired_revision === node.applied_revision; return <div className="node-row"><div className="node-name"><div className="node-avatar"><Server /></div><div><strong>{node.name}</strong><span>{profile?.name ?? "Без профиля"}</span></div></div><div className="node-row__revision"><span className="mono">rev {node.applied_revision}</span><Badge tone={node.status === "error" ? "danger" : synced && node.status === "online" ? "success" : "warning"}>{node.status === "error" ? "ошибка" : synced ? "актуально" : "ожидает"}</Badge></div></div> }
