...
function Profiles({ profiles, nodes, onCreate, onOpen, onChanged }: { profiles: Profile[]; nodes: NodeInfo[]; onCreate: () => void; onOpen: (profile: Profile) => void; onChanged: () => Promise<void> }) {
  return <div className="page"><PageHeader eyebrow="Desired state" title="Профили" description="Один профиль можно назначить нескольким нодам." action={<Button onClick={onCreate}><Plus data-icon="inline-start" />Новый профиль</Button>} />
    <div className="profile-grid">{profiles.map((profile) => { const assigned = nodes.filter((node) => node.profile_id === profile.id).length; return <Card key={profile.id} className="profile-card" onClick={() => onOpen(profile)} role="button" tabIndex={0} onKeyDown={(e) => (e.key === "Enter" || e.key === " ") && onOpen(profile)}><div className="profile-card__top"><div className="profile-icon"><Boxes /></div><div className="profile-card__actions"><Badge>rev {profile.current_revision}</Badge><Button variant="ghost" className="icon-button" title="Клонировать профиль" aria-label={`Клонировать ${profile.name}`} onClick={async (event) => { event.stopPropagation(); const name = window.prompt("Название копии", `${profile.name} — копия`); if (!name) return; await api.cloneProfile(profile.id, name); await onChanged() }}><Copy /></Button><Button variant="ghost" className="icon-button danger-hover" disabled={assigned > 0} title={assigned ? "Сначала назначьте нодам другой профиль" : "Удалить профиль"} aria-label={`Удалить ${profile.name}`} onClick={async (event) => { event.stopPropagation(); if (!window.confirm(`Удалить профиль «${profile.name}»?`)) return; await api.deleteProfile(profile.id); await onChanged() }}><Trash2 /></Button></div></div><div><h2>{profile.name}</h2><p>{profile.description || "Без описания"}</p></div><div className="profile-card__meta"><span>{assigned} нод</span><span>Изменён {formatRelative(profile.updated_at)}</span></div></Card> })}</div>
  </div>
}
...
function Nodes({ nodes, profiles, onChanged }: { nodes: NodeInfo[]; profiles: Profile[]; onChanged: () => Promise<void> }) {
  ...
  const installCommand = credential ? `curl -fLO https://github.com/ezhikdev/ezhiklb/releases/download/v0.1.0-alpha.6/ezhiklb_0.1.0-alpha.6_linux_amd64.tar.gz && tar -xzf ezhiklb_0.1.0-alpha.6_linux_amd64.tar.gz && sudo EZHIKLB_ROLE=node EZHIKLB_PANEL_URL=${panelURL.replace(/\/$/, "")} EZHIKLB_NODE_ID=${credential.node.id} EZHIKLB_AGENT_TOKEN=${credential.token} ./install.sh` : ""
  const create = async () => { setBusy(true); try { const result = await api.createNode(name, address, profileID); setCredential({ node: result.node, token: result.agent_token }); await onChanged() } finally { setBusy(false) } }
  return <div className="page"><PageHeader eyebrow="Infrastructure" title="Ноды" description="Назначайте каждой ноде один из переиспользуемых профилей." action={<Button onClick={() => setAdding(true)}><Plus data-icon="inline-start" />Добавить ноду</Button>} />
    <div className="notice"><ShieldCheck /><div><strong>Отдельный ключ для каждой ноды</strong><span>Ключ показывается один раз. Для удалённого сервера используй HTTPS; открытый HTTP допустим только с явно включённым тестовым insecure-режимом.</span></div></div>
    <Card className="table-card"><div className="node-table">{nodes.map((node) => <div className="node-table__row" key={node.id}><div className="node-name"><div className="node-avatar"><Server /></div><div><strong>{node.name}</strong><span className="mono">{node.ingress_address || "auto-detect"} · {node.agent_version || "агент не подключался"}</span></div></div><Badge tone={node.status === "online" ? "success" : node.status === "error" ? "danger" : "neutral"}>{node.status}</Badge><label className="compact-select"><span>Профиль</span><select value={node.profile_id} onChange={async (e) => { await api.assignProfile(node.id, e.target.value); await onChanged() }}>{profiles.map((profile) => <option key={profile.id} value={profile.id}>{profile.name}</option>)}</select></label><div className="node-actions"><div className="revision-state"><span>desired {node.desired_revision}</span><span>actual {node.applied_revision}</span><span>{node.last_seen_at ? formatRelative(node.last_seen_at) : "ещё не подключалась"}</span></div><Button variant="ghost" className="icon-button" title="Изменить ноду" aria-label={`Изменить ${node.name}`} onClick={async () => { const next = window.prompt("Название ноды", node.name); if (!next) return; const nextAddress = window.prompt("Публичный IPv4 ноды (можно оставить пустым)", node.ingress_address) ?? node.ingress_address; await api.updateNode(node.id, next, nextAddress); await onChanged() }}><Pencil /></Button>{node.id !== "local" && <><Button variant="ghost" className="icon-button" title={node.status === "disabled" ? "Включить и создать новый ключ" : "Создать новый ключ"} aria-label={`Сменить ключ ${node.name}`} onClick={async () => { if (!window.confirm("Старый ключ сразу перестанет работать. Продолжить?")) return; const result = await api.rotateNodeToken(node.id); setCredential({ node, token: result.agent_token }); setAdding(true); await onChanged() }}><KeyRound /></Button>{node.status !== "disabled" && <Button variant="ghost" className="icon-button" title="Отключить ноду и отозвать ключ" aria-label={`Отключить ${node.name}`} onClick={async () => { if (!window.confirm(`Отключить ноду «${node.name}»? Её ключ будет отозван.`)) return; await api.revokeNode(node.id); await onChanged() }}><Power /></Button>}<Button variant="ghost" className="icon-button danger-hover" title="Удалить ноду" aria-label={`Удалить ${node.name}`} onClick={async () => { if (!window.confirm(`Удалить ноду «${node.name}» из панели?`)) return; await api.deleteNode(node.id); await onChanged() }}><Trash2 /></Button></>}</div></div>)}</div></Card>
    {adding && <Dialog title={credential ? "Ключ и команда установки" : "Новая нода"} description={credential ? "Скопируй сейчас: после закрытия ключ больше не показывается." : "Создайте ноду и назначьте ей существующий профиль."} onClose={() => { setAdding(false); setCredential(null) }}>{credential ? <><div className="dialog__body node-enrollment"><div className="credential-warning"><KeyRound /><div><strong>Секрет ноды</strong><span>Не публикуй эту команду и не используй её для другой ноды.</span></div></div><Field label="Публичный URL панели" hint="URL должен быть доступен именно с новой VPS; 127.0.0.1 из SSH-туннеля здесь не подойдёт"><Input className="input mono" value={panelURL} onChange={(event) => setPanelURL(event.target.value)} /></Field><Field label="Команда установки на новой VPS"><textarea className="command-box mono" readOnly value={installCommand} rows={7} onFocus={(event) => event.currentTarget.select()} /></Field><div className="notice"><Network /><div><strong>Если тестовая панель открыта по HTTP</strong><span>Добавь перед ./install.sh переменную EZHIKLB_ALLOW_INSECURE=1. В production настрой HTTPS и не добавляй её.</span></div></div></div><div className="dialog__footer"><Button onClick={() => { void navigator.clipboard?.writeText(installCommand) }}><Copy data-icon="inline-start" />Копировать</Button><Button variant="secondary" onClick={() => { setAdding(false); setCredential(null) }}>Готово</Button></div></> : <><div className="dialog__body form-stack"><Field label="Название"><Input value={name} onChange={(event) => setName(event.target.value)} /></Field><Field label="Публичный IPv4" hint="Нужен для правил NAT; можно заполнить позже"><Input className="input mono" placeholder="203.0.113.10" value={address} onChange={(event) => setAddress(event.target.value)} /></Field><Field label="Профиль"><select value={profileID} onChange={(event) => setProfileID(event.target.value)}>{profiles.map((profile) => <option key={profile.id} value={profile.id}>{profile.name}</option>)}</select></Field></div><div className="dialog__footer"><Button variant="ghost" onClick={() => setAdding(false)}>Отмена</Button><Button disabled={busy || !name.trim() || !profileID} onClick={create}>{busy ? "Создаю…" : "Создать ноду"}</Button></div></>}</Dialog>}
  </div>
}
