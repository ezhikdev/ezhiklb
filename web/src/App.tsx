...
function Nodes({ nodes, profiles, onChanged }: { nodes: NodeInfo[]; profiles: Profile[]; onChanged: () => Promise<void> }) {
  ...
  const [busy, setBusy] = useState(false)
  const installCommand = credential ? `curl -fLO https://github.com/ezhikdev/ezhiklb/releases/download/v0.1.0-alpha.7/ezhiklb_0.1.0-alpha.7_linux_amd64.tar.gz && tar -xzf ezhiklb_0.1.0-alpha.7_linux_amd64.tar.gz && sudo EZHIKLB_ROLE=node EZHIKLB_PANEL_URL=${panelURL.replace(/\/$/, "")} EZHIKLB_NODE_ID=${credential.node.id} EZHIKLB_AGENT_TOKEN=${credential.token} ./install.sh` : ""
