import { Activity, ArrowDown, ArrowUp, Boxes, CheckCircle2, ChevronDown, ChevronRight, CircleAlert, CircleGauge, Clock3, Copy, Cpu, Github, GripVertical, HeartPulse, Hexagon, History, LoaderCircle, LogOut, MemoryStick, Network, Pencil, Plus, Power, RefreshCw, Save, ScrollText, Server, Settings, ShieldCheck, Trash2, Users, X } from "lucide-react"
...
const resetMinAgentVersion = "1.0.7"
const rateLimitMinAgentVersion = "1.1.0"
...
// A node stuck in "deleting" has no automatic timeout — its agent may be gone
// for good (replaced by a fresh enrollment, VPS reinstalled, etc.) and will
// never confirm cleanup. Give the normal flow this long to self-resolve
// before offering the force-delete escape hatch; `updated_at` keeps sliding
// forward on every heartbeat, so a genuinely still-working decommission never
// shows the button prematurely.
const FORCE_DELETE_GRACE_MS = 60_000
function forceDeleteEligible(node: NodeInfo): boolean {
...
function Profiles({ profiles, nodes, onCreate, onOpen, onChanged }: { profiles: Profile[]; nodes: NodeInfo[]; onCreate: () => void; onOpen: (profile: Profile) => void; onChanged: () => Promise<void> }) {
  ...
  const profileDrag = useDragReorder({
    items: orderedProfiles,
    getID: (profile) => profile.id,
    axis: "grid",
    onReorder: setOrderedProfiles,
    onCommit: async (items) => { setOrderError(""); await api.reorderProfiles(items.map((profile) => profile.id)); await onChanged() },
    onError: (reason) => setOrderError(reason instanceof Error ? reason.message : "Не удалось сохранить порядок профилей"),
  })
  useEffect(() => { if (!profileDrag.activeID) setOrderedProfiles(profiles) }, [profiles, profileDrag.activeID])
  return <div className="page"><PageHeader eyebrow="Desired state" title="Профили" description="Один профиль можно назначить нескольким нодам." action={<Button onClick={onCreate}><Plus data-icon="inline-start" />Новый профиль</Button>} />
    {orderError && <div className="validation-error" role="alert">{orderError}</div>}
    <div className="profile-grid">{orderedProfiles.map((profile) => {
      const assigned = nodes.filter((node) => node.profile_id === profile.id).length
      return <div key={profile.id} ref={profileDrag.itemRef(profile.id)} className={`profile-sortable ${profileDrag.activeID === profile.id ? "sortable-placeholder" : ""}`}><Card className="profile-card" onClick={() => onOpen(profile)} role="button" tabIndex={0} onKeyDown={(event) => (event.key === "Enter" || event.key === " ") && onOpen(profile)}>
        <div className="profile-card__top"><div className="profile-icon"><Boxes /></div><div className="profile-card__actions"><button type="button" className="sort-handle" aria-label={`Изменить порядок: ${profile.name}`} title="Перетащите профиль" onClick={(event) => event.stopPropagation()} {...profileDrag.handleProps(profile.id)}><GripVertical /></button><Badge>{profile.version}</Badge><Button variant="ghost" className="icon-button" title="Клонировать профиль" aria-label={`Клонировать ${profile.name}`} onClick={(event) => { event.stopPropagation(); setCloning(profile); setCloneName(`${profile.name} — копия`) }}><Copy /></Button><Button variant="ghost" className="icon-button danger-hover" disabled={assigned > 0} title={assigned ? "Сначала назначьте нодам другой профиль" : "Удалить профиль"} aria-label={`Удалить ${profile.name}`} onClick={(event) => { event.stopPropagation(); setDeleting(profile) }}><Trash2 /></Button></div></div>
...
function Nodes({ nodes, profiles, settings, stats, health, onChanged }: { nodes: NodeInfo[]; profiles: Profile[]; settings: SystemSettings; stats: ServiceStat[]; health: BackendHealth[]; onChanged: () => Promise<void> }) {
  ...
  const nodeDrag = useDragReorder({
    items: orderedNodes,
    getID: (node) => node.id,
    axis: "vertical",
    onReorder: setOrderedNodes,
    onCommit: async (items) => { setOrderError(""); await api.reorderNodes(items.map((node) => node.id)); await onChanged() },
    onError: (reason) => setOrderError(reason instanceof Error ? reason.message : "Не удалось сохранить порядок нод"),
  })
  useEffect(() => { if (!nodeDrag.activeID) setOrderedNodes(nodes) }, [nodes, nodeDrag.activeID])
  return <div className="page"><PageHeader eyebrow="Infrastructure" title="Ноды" description="Подключение, состояние и применение профилей на всех серверах." action={<Button onClick={beginAdd}><Plus data-icon="inline-start" />Добавить ноду</Button>} />
    ...
    {orderError && <div className="validation-error" role="alert">{orderError}</div>}
    <Card className="table-card"><div className="node-table">{orderedNodes.map((node) => {
      ...
      return <div ref={nodeDrag.itemRef(node.id)} className={`node-table__row ${node.status === "disabled" ? "node-table__row--disabled" : ""} ${locked ? "node-table__row--deleting" : ""} ${nodeDrag.activeID === node.id ? "sortable-placeholder" : ""}`} key={node.id}>
        <button type="button" className="sort-handle node-sort-handle" aria-label={`Изменить порядок: ${node.name}`} title="Перетащите ноду" disabled={locked} {...nodeDrag.handleProps(node.id)}><GripVertical /></button>
        ...
        <div className="node-actions"><div className="revision-state"><span>{applyStateLabel(node)}</span><span>{assignedProfile?.version || "версия неизвестна"}</span>{node.update_state === "error" && <span className="revision-error" title={node.update_error}>{node.update_error}</span>}{!updating && isOlderVersion(node.agent_version, releaseVersion) && node.status === "online" && (supportsOneClickUpdate ? <Button variant="secondary" className="node-update-button" onClick={() => setConfirmNode({ node, action: "update" })}><RefreshCw />Обновить до {releaseVersion}</Button> : <span className="node-update-legacy" title="В версиях до beta.3.3 валидатор ошибочно отклоняет имя beta-релиза">Первое обновление — вручную</span>)}{locked && forceDeleteEligible(node) && <Button variant="secondary" className="node-force-delete-button" onClick={() => setConfirmNode({ node, action: "force-delete" })}><CircleAlert />Удалить принудительно</Button>}{node.last_error && <span className="revision-error" title={node.last_error}>{node.last_error}</span>}</div>{!locked && <div className={`node-action-buttons ${node.id === "local" ? "node-action-buttons--local" : ""}`}><Button variant="ghost" className="icon-button" title="Изменить" aria-label={`Изменить ${node.name}`} onClick={() => { setEditingNode(node); setEditName(node.name) }}><Pencil /></Button><Button variant="ghost" className="icon-button" title={node.status === "disabled" ? "Включить" : "Выключить"} aria-label={`${node.status === "disabled" ? "Включить" : "Выключить"} ${node.name}`} onClick={() => { void changeEnabled(node) }}><Power /></Button>{node.id !== "local" && <Button variant="ghost" className="icon-button danger-hover" title="Удалить" aria-label={`Удалить ${node.name}`} onClick={() => setConfirmNode({ node, action: "delete" })}><Trash2 /></Button>}</div>}</div>
    ...
    {confirmNode && <ConfirmDialog title={confirmNode.action === "delete" ? "Удалить ноду?" : confirmNode.action === "force-delete" ? "Удалить принудительно?" : confirmNode.action === "update" ? "Обновить агент ноды?" : "Выключить ноду?"} description={confirmNode.action === "delete" ? `EzhikLB очистит свои маршруты на «${confirmNode.node.name}», остановит агент и удалит ноду из панели.` : confirmNode.action === "force-delete" ? `Нода «${confirmNode.node.name}» не подтвердила удаление и не отвечает уже больше минуты. Панель уберёт запись без подтверждения от неё. Если агент на VPS всё ещё жив, но недоступен панели, его IPVS- и firewall-правила не будут очищены автоматически — проверьте вручную (ipvsadm -Ln, iptables -S EZHIKLB-FORWARD) или переустановите агент.` : confirmNode.action === "update" ? `Нода «${confirmNode.node.name}» проверит официальный релиз ${releaseVersion}, заменит агент и автоматически перезапустит его. Маршруты IPVS продолжат работать.` : `Нода «${confirmNode.node.name}» перестанет принимать новые настройки панели. Текущая балансировка продолжит работать.`} confirmLabel={confirmNode.action === "delete" ? "Удалить ноду" : confirmNode.action === "force-delete" ? "Удалить принудительно" : confirmNode.action === "update" ? "Обновить ноду" : "Выключить"} danger={confirmNode.action === "delete" || confirmNode.action === "force-delete"} busy={busy} onCancel={() => setConfirmNode(null)} onConfirm={async () => { setBusy(true); try { if (confirmNode.action === "delete") await api.deleteNode(confirmNode.node.id); else if (confirmNode.action === "force-delete") await api.forceDeleteNode(confirmNode.node.id); else if (confirmNode.action === "update") await api.requestNodeUpdate(confirmNode.node.id); else await api.setNodeEnabled(confirmNode.node.id, false); setConfirmNode(null); await onChanged() } catch (reason) { notify({ tone: "danger", text: reason instanceof Error ? reason.message : "Не удалось выполнить действие" }, 6000) } finally { setBusy(false) } }} />}
...
function ProfileDialog({ existing, health, nodes, onClose, onSaved }: { existing: { profile: Profile; revision: Revision } | null; health: BackendHealth[]; nodes: NodeInfo[]; onClose: () => void; onSaved: () => Promise<void> }) {
  ...
  const profileNodes = existing ? nodes.filter((node) => node.profile_id === existing.profile.id) : []
  const incompatibleResetNodes = profileNodes.filter((node) => !node.agent_version || isOlderVersion(node.agent_version, resetMinAgentVersion))
  const resetAvailable = profileNodes.length > 0 && incompatibleResetNodes.length === 0
  const hasRateLimits = config.listeners.some((listener) => listener.enabled && listener.rate_limit_enabled && (listener.rate_limit_mbps ?? 0) > 0)
  const incompatibleRateLimitNodes = hasRateLimits ? profileNodes.filter((node) => !node.agent_version || isOlderVersion(node.agent_version, rateLimitMinAgentVersion)) : []
  const manualVersionValid = /^[A-Za-z0-9][A-Za-z0-9.-]{0,63}$/.test(version) && (!existing || version !== existing.profile.version)
  const dirty = name !== initialName || description !== initialDescription || autoVersion !== (existing?.profile.auto_version ?? true) || version !== (existing?.profile.version ?? "v1") || resetConnections || JSON.stringify(config) !== JSON.stringify(initialConfig)
  const close = () => { if (!dirty) { onClose(); return } setConfirmClose(true) }
  const save = async () => { setBusy(true); setError(""); try { if (existing) await api.publishProfile(existing.profile.id, name, description, config, autoVersion, autoVersion ? "" : version, resetConnections); else await api.createProfile(name, description, config, autoVersion, autoVersion ? "" : version); await onSaved() } catch (reason) { setError(reason instanceof Error ? reason.message : "Не удалось сохранить профиль") } finally { setBusy(false) } }
...
function eventLabel(action: string) { return ({ "profile.created": "Профиль создан", "profile.published": "Профиль опубликован", "profile.rolled_back": "Профиль восстановлен", "profile.deleted": "Профиль удалён", "node.created": "Нода создана", "node.updated": "Нода изменена", "node.profile_assigned": "Профиль назначен ноде", "node.enabled_changed": "Состояние ноды изменено", "node.decommission_requested": "Запрошено удаление ноды", "node.decommissioned": "Нода очищена и удалена", "node.force_deleted": "Нода удалена принудительно (без подтверждения)", "node.apply_failed": "Ошибка применения конфигурации", "node.apply_recovered": "Конфигурация снова применена", "node.credential_rotated": "Токен ноды обновлён", "node.credential_revoked": "Токен ноды отозван", "node.health_probe_requested": "Запрошена внеплановая проверка", "node.update_requested": "Запрошено обновление агента", "settings.updated": "Сетевые настройки изменены" } as Record<string, string>)[action] ?? action }
