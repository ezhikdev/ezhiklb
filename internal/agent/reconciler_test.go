...
func TestTrafficControlIsStrictlyOptIn(t *testing.T) {
	runner := &fakeRunner{}
	r := NewReconciler(runner, filepath.Join(t.TempDir(), "state.json"), nil)
	services := []Service{{ListenerID: "plain", Protocol: domain.ProtocolTCP, Address: "198.51.100.10", Port: 443}}
	controls, err := r.reconcileTrafficControl(context.Background(), nil, services)
	if err != nil {
		t.Fatal(err)
	}
	if len(controls) != 0 {
	...
	if len(runner.calls) != 0 {
		t.Fatalf("unlimited services invoked network mutation: %#v", runner.calls)
	}
}

func TestTrafficControlSharesLimitAcrossProtocolsAndSeparatesDirections(t *testing.T) {
	runner := &fakeRunner{}
	r := NewReconciler(runner, filepath.Join(t.TempDir(), "state.json"), nil)
	destination := Destination{ID: "backend", Address: "192.0.2.10", Port: 9000, Weight: 1}
	services := []Service{
		{ListenerID: "limited", Protocol: domain.ProtocolTCP, Address: "198.51.100.10", Port: 443, RateLimitEnabled: true, RateLimitMbps: 450, Destinations: []Destination{destination}},
		{ListenerID: "limited", Protocol: domain.ProtocolUDP, Address: "198.51.100.10", Port: 443, RateLimitEnabled: true, RateLimitMbps: 450, Destinations: []Destination{destination}},
	}
	controls, err := r.reconcileTrafficControl(context.Background(), nil, services)
	if err != nil {
		t.Fatal(err)
	...
	if len(controls) != 2 {
		t.Fatalf("controls = %#v, want one per direction", controls)
	}
	if controls[0].Mark == controls[1].Mark {
		t.Fatal("upload and download unexpectedly share one limiter")
	}
	var filterCommands int
	var marks string
	for _, call := range runner.calls {
		if call.name == "tc" && len(call.args) > 1 && call.args[0] == "filter" && call.args[1] == "replace" {
			filterCommands++
			if !strings.Contains(strings.Join(call.args, " "), "rate 450mbit") {
				t.Fatalf("wrong tc rate: %#v", call.args)
			}
		}
		if call.name == "iptables-restore" && strings.Contains(call.input, "EZHIKLB-RATE") {
			marks = call.input
		}
	}
	if filterCommands != 2 {
		t.Fatalf("tc filter commands = %d, want 2: %#v", filterCommands, runner.calls)
	}
	if !strings.Contains(marks, "--vproto tcp") || !strings.Contains(marks, "--vproto udp") || !strings.Contains(marks, "--vdir ORIGINAL") || !strings.Contains(marks, "--vdir REPLY") {
		t.Fatalf("incomplete IPVS rate marks: %s", marks)
	}
}
...
func TestSaveHealthCheckPreservesAppliedDataPlane(t *testing.T) {
	...
	if len(state.Services) != 1 || state.Services[0].Port != 8002 {
		t.Fatalf("services changed: %#v", state.Services)
	}
	if state.HealthCheck.IntervalSeconds != 30 || !state.HealthCheck.Enabled {
		t.Fatalf("health-check was not persisted: %#v", state.HealthCheck)
	}
}
